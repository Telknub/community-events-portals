...
export const BumpkinProfile: React.FC = () => {
  ...
  const { collected, lives, maxLives, selectedWeapon, weaponLevels } =
    useSelector(portalService, _profileState);

  const healthPercent = maxLives > 0 ? (lives / maxLives) * 100 : 0;
  const currentHealth = Math.max(0, Math.round(healthPercent));
  ...
  return (
    <>
      <div className="flex flex-col items-end gap-1" style={{ width: "154px" }}>
        <button
          type="button"
          className="w-full cursor-pointer rounded-md bg-blueGray-900 bg-opacity-80 p-2 text-white text-shadow hover:brightness-110"
          onClick={() => {
            setInspectedWeapon(selectedWeapon);
            setShowModal(true);
          }}
        >
          <div className="mb-1 flex items-center justify-between gap-2">
            <div className="flex items-center gap-1">
              <SquareIcon icon={SUNNYSIDE.icons.heart} width={8} />
              <span className="text-xs">{"HP"}</span>
            </div>
            <span className="text-xs">{`${currentHealth}/100`}</span>
          </div>
          <ProgressBar
            percent={healthPercent}
            color={healthPercent > 35 ? "#59c36a" : "#d24d4d"}
          />
        </button>
      </div>
