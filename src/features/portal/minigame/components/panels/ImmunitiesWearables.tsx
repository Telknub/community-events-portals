...
import { AURA_IMMUNITY, SHIRT_IMMUNITY, SHOES_IMMUNITY, HAT_IMMUNITY } from "../../Constants";
import { ImmunityTooltip } from "./ImmunityTooltip";

export const Immunities_Wearables: React.FC = () => {
  const { aura: equippedAura, shirt: equippedShirt, shoes: equippedShoe, hat: equippedHat } =
    STATIC_OFFLINE_FARM.bumpkin.equipped;

  const aura = equippedAura && AURA_IMMUNITY.includes(equippedAura) ? equippedAura : undefined;
  const shirt = equippedShirt && SHIRT_IMMUNITY.includes(equippedShirt) ? equippedShirt  : undefined;
  ...
  if (!aura && !shirt && !shoe && !hat) return null;
  ...
  return (
    <div className="flex flex-row mb-5 text-sm">
      ...
      {shirt && (
        <ImmunityTooltip
          id="shirt"
          image={getWearableImage(shirt)}
          description={STATIC_OFFLINE_FARM.bumpkin.equipped.shirt}
          openId={openId}
          setOpenId={setOpenId}
        />
      )}
