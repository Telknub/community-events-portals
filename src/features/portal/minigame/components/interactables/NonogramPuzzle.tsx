...
export const NonogramPuzzle: React.FC<Props> = ({ onClose, onAction }) => {
  ...
  return (
    <>
       <div className="fixed inset-0 bg-white-200 z-0 backdrop-blur">
       <div className="relative text-white flex flex-col items-center justify-center w-full h-full">
      ...
      <div className="flex ml-[55px] sm:ml-[40px] md:ml-[80px]">
        {colClues.map((clue, i) => (
          ...
          > 
            {clue.map((num, j) => (
              <div className="my-2" key={j}>{num}</div>
            ))}
          </div>
        ))}
      </div>
          ...
          <div key={r} className="flex items-center justify-center">
            ...
            ">
              {rowClues[r].map((num, i) => (
                <span key={i} className="mx-1">{num}</span>
              ))}
            </div>
              ...
              {PATTERNS[r].map((_, c) => {
                ...
                return (
                  <div
                    ...
                    className={`
                      border-2 sm:border-3 md:border-4 border-green-700
                      cursor-pointer transition
                      ${isFilled ? "bg-green-700" : "bg-green-100"}
                      w-[40px] h-[40px]
                      sm:w-[50px] sm:h-[50px]
                      md:w-[70px] md:h-[70px]
                    `}
